...
> 학습시 마다 설정과 경과등을 기록하였으나 사내 프로젝트인 관계로 해당 부분 문서는 공개하지 않으며, 이론 정리한 내용만 업로드되어 있습니다.
...
* 사용할 모델 : Cotatron
* 목표 
	* 이 모델은 기존에 영어를 기준으로 개발되었으므로, 공공데이터를 활용하여 한국어 셋에 맞도록 새로 학습시키고 하이퍼 파라미터를 찾는다. (현제 여기까지 진행)
